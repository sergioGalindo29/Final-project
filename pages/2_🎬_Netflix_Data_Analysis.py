...
countries_df = movies_df['country'].str.split(', ').explode()

n_countries = len(countries_df.unique())


# TODO: Ex 2.5: How many characters long are on average the title names?
movies_df['title_length'] = movies_df['title'].apply(lambda x: len(str(x)))
...
movies_df['duration_minutes'] = movies_df.loc[movies_df['type'] == 'Movie', 'duration'].apply(lambda x: int(x.split(' ')[0]) if pd.notna(x) else None)
movies_avg_duration_per_year = movies_df.groupby('release_year')['duration_minutes'].mean()

if movies_avg_duration_per_year is not None:
    fig = plt.figure(figsize=(9, 6))

    plt.plot(movies_avg_duration_per_year.index, movies_avg_duration_per_year.values, linestyle='-', color='b')   # TODO: generate the line plot using plt.plot() and the information from movies_avg_duration_per_year (the vertical axes with the minutes value) and its index (the horizontal axes with the years)
